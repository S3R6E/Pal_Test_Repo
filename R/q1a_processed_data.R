...
#create unique functional groups to fill in with zero
...
#merge data with filler and replace NA with zero
data_rc_cover <-
  data_rc_cover |> 
  full_join(filler) |>
  group_by(
...
#summarize functional group per image 
data_rc_cover <- 
  data_rc_cover |>
...
#rename group and count columns
data_rc_cover <- 
  data_rc_cover |> 
...
#converts into data frame format
data_rc_cover |> as.data.frame() |> glimpse()

#filter groups that are only HC and create columns for cover and date
data_rc_cover <- 
  data_rc_cover |> 
...
##create a lookup table for west and east side
...
save(data_rc_cover, file = "../data/processed/q1a_data_rc_cover.RData")
